...
│  DS-K1T341AMF   │───▶│  backend/        │───▶│  Firestore +    │
...
```
final-face/                         # Main repo (icannDevTeam/final-face)
├── backend/                        # Python backend
│   ├── main.py                     # Core face recognition engine
│   ├── attendance_listener.py      # Hikvision event stream monitor
│   ├── hikvision_attendance.py     # Device management
│   ├── api_integrate.py            # BINUS API bridge
│   ├── student_metadata.py         # Student ID mapping
│   ├── requirements.txt            # Python dependencies
│   └── data/                       # Runtime data (gitignored)
├── mobile-attendance/              # Vite + React PWA
├── web-dataset-collector/          # Next.js 14 dashboard [git submodule → icannDevTeam/dataset]
...
|-------------------------|-----------------------------------|--------------------------------------|
| Face recognition engine | `backend/main.py`                 | OpenCV + dlib face processing (2578 lines) |
| Attendance listener     | `backend/attendance_listener.py`  | Live Hikvision event stream monitor  |
| Hikvision management    | `backend/hikvision_attendance.py` | Device enrollment, sync, management  |
| Dataset collector       | `web-dataset-collector/`          | Next.js app for photo capture/upload (submodule) |
| Mobile attendance       | `mobile-attendance/`              | Vite PWA with face-api.js            |
| API integration         | `backend/api_integrate.py`        | BINUS School API bridge              |
| Student metadata        | `backend/student_metadata.py`     | employeeNo → BINUS ID mapping        |
| Monitoring stack        | `monitoring/`                     | Prometheus + Grafana                 |
| Stress testing          | `backend/stress_test.py`          | Load/performance testing             |
...
- `backend/data/student_metadata.json` — Student records (employeeNo, binusId, name)
- `backend/data/attendance/*.json` — Daily attendance logs (by date)
- `backend/requirements.txt` — Python dependencies
- `facial-attendance-binus-firebase-adminsdk.json` — Firebase service account (DO NOT COMMIT)
- `.env` — Shared environment secrets (symlinked into `backend/` and `web-dataset-collector/`)

## Conventions

- Python backend lives in `backend/` — standalone modules (one file = one concern)
- Run Python scripts from `backend/` directory: `cd backend && python3 script.py`
- Next.js pages in `web-dataset-collector/pages/`, API routes in `pages/api/`
- Local JSON persistence in `backend/data/` directory
- Firebase for cloud state, local JSON for edge/backup
- Hikvision integration: Python in `backend/`, Node.js in `web-dataset-collector/lib/hikvision.js`
- `web-dataset-collector/` is a git submodule — commit changes there separately
- Shared config (`.env`, Firebase JSON) stays at project root, symlinked into subfolders

## Known Gotchas

- Hikvision device event *search* endpoint is not supported — must use live event stream
- `backend/main.py` is large (2578 lines) — the core face recognition engine, handles multiple modes
...
- `backend/` has symlinks to root `.env` and `facial-attendance-binus-firebase-adminsdk.json`
- `web-dataset-collector/` is a git submodule — `git pull` won't update it; use `git submodule update --remote`
...
| **Hardcoded Hikvision password fallback** | `password.123` as env fallback in `backend/attendance_listener.py:51`, `backend/hikvision_attendance.py:57`, `backend/capture_and_upload.py:45` |
...
| **Student IDs in stress test** | Real student IDs hardcoded in `backend/stress_test.py:28-34` — data privacy concern in source control |
